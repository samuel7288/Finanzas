...
import { DashboardModel, GmailStatus } from "../types";
        ...
        <aside className="dashboard-sidebar">
          ...
          <nav className="sidebar-nav" aria-label="Principal">
            {model.navItems.map((item) => (
              <button key={item.id} className={item.active ? "nav-item active" : "nav-item"}>
          ...
          <section className="sidebar-card">
            ...
            <div className="gmail-connection">
              ...
              <div>
                <strong>{model.connectedLabel}</strong>
                <p>{model.connectedHelper}</p>
                <span className={`connection-state ${model.connectionState}`}>
                  {model.connectionState === "active"
                    ? "Activo"
          ...
          <section className="sidebar-card premium-card">
            ...
            <button className="primary-action">Actualizar</button>
          </section>
              ...
              <section className="metrics-grid">
                {model.metrics.map((metric) => (
                  <article key={metric.id} className="surface metric-card">
                    ...
                    <span className={metric.deltaPositive ? "metric-delta positive" : "metric-delta negative"}>
                  ...
                  <div className="recent-list">
                    {model.recentTransactions.map((item) => (
                      <article key={item.id} className="recent-row">
                        ...
                        <span className={item.positive ? "recent-amount positive" : "recent-amount negative"}>
                          {item.amountLabel}
                        </span>
                      </article>
                    ))}
...
function Sparkline({ series, accent }: { series: number[]; accent: string }) {
  ...
  return (
    <svg className="sparkline" viewBox="0 0 260 56" preserveAspectRatio="none">
      <defs>
        <linearGradient id={`spark-${accent.replace("#", "")}`} x1="0%" x2="100%">
          <stop offset="0%" stopColor={accent} stopOpacity="0" />
          <stop offset="100%" stopColor={accent} stopOpacity="0.28" />
        </linearGradient>
      </defs>
      <path d={`${path} L254 54 L6 54 Z`} fill={`url(#spark-${accent.replace("#", "")})`} />
